...
class Api:
    ...
    def __init__(self):
        ...
        self.departements = sorted(list(set([departement["departement"] for departement in self.getLines(select=["departement"], size=self.maxlines)])))
        self.regions = sorted(list(set([region["region"] for region in self.getLines(select=["region"], size=self.maxlines)])))
    ...
    def getCO2(self, type_structure:str, nom:str):
        ...
        match type_structure:
            ...
            case "Départements":
                lines = self.getLines(select=["date_de_publication"]+params, size=self.maxlines, qs=f"departement:\"{nom}\"")
            case "Régions":
                lines = self.getLines(select=["date_de_publication"]+params, size=self.maxlines, qs=f"region:\"{nom}\"")
            case _:
                lines = self.getLines(select=["date_de_publication"]+params, size=self.maxlines, qs=f"type_de_structure: Collectivité territoriale AND type_de_collectivite:{type_structure} AND raison_sociale:\"{nom}\"")
    ...
    def getCO2Total(self, type_structure:str):
        ...
        match type_structure:
            case "Départements":
                lines = self.getLines(select=["departement"]+params, size=self.maxlines)
                dataname = "departement"
            case "Régions":
                lines = self.getLines(select=["region"]+params, size=self.maxlines)
                dataname = "region"
            case _:
                lines = self.getLines(select=["departement"]+params, size=self.maxlines)
